# Inner Bloom Result Page — Native Shopify HTML/CSS v11 Mobile Conversion
# Page: Inner Bloom — Your Wellness Match
# URL handle: inner-bloom-result
# Vital Vision Shop
# Generated: 2026-05-14
# Version: v13-balanced-mobile-hero
# Base: v12-hero-shorter-mobile — same structure, mobile hero image height restored slightly
...
#
# Changes from v10-final-clean:
#   REMOVED  — Trust badge grid (GMP Certified, Third-Party Tested, Gluten/Dairy/Soy Free,
#              Quality-focused). These are unverified claims per current rules.
#   REPLACED — Trust strip now shows only the required FDA compliance note (no badges).
#   REDUCED  — Mobile product image: max-width 140px → 110px, min-height 180px → 150px
#   REDUCED  — Product card section top padding: 32px → 24px (card sits closer to hero)
#   UNCHANGED — Hero structure, copy, CTA, object-position: 80% 66%, margin-top: -72px
#   UNCHANGED — Why section, ritual steps, FAQ, final CTA bar
#   UNCHANGED — Desktop layout (all desktop overrides preserved)
#   NO GLOBAL CHANGES — no theme files, no header, no footer, no homepage
...
```html
<!-- ============================================================
     VV Inner Bloom Result Page — Native HTML/CSS v13 Balanced Mobile Hero
     ...
     No unverified trust claims. FDA disclaimer present.
     ============================================================ -->
...
.vvp8 {
  ...
  /* Cancels Refresh section top padding. Adjust if gap appears above hero. */
  margin-top: -72px;
}
...
.v8-section-sm { width: 100%; padding: 24px 20px; } /* tighter for product card section */
...
/* ══════════════════════════════════════════════════════════════
   ...
   Mobile: lifestyle image top (175px), copy below.
...
.v8-hero-img-col { order: 1; overflow: hidden; height: 152px; } /* BALANCED: 128px was too short, restored to 152px */
.v8-hero-img {
  width: 100%; height: 100%; display: block;
  object-fit: cover;
  /* Mobile crop: 80% right, 66% down — woman + glass visible. */
  object-position: 80% 66%;
}
.v8-hero-copy {
  order: 2; padding: 14px 20px 18px; /* REDUCED: was 24px/32px */
  display: flex; flex-direction: column; align-items: flex-start;
}
.v8-hero-h1 {
  font-size: 22px; font-weight: 500; line-height: 1.15; /* REDUCED: was 24px */
  color: #FFFFFF; margin-bottom: 10px; letter-spacing: -.01em;
}
.v8-hero-sub {
  font-size: 14px; line-height: 1.5; /* REDUCED: was 1.6 */
  color: rgba(255,248,236,.85);
  margin-bottom: 18px; max-width: 340px; /* REDUCED: was 28px */
}

/* ══════════════════════════════════════════════════════════════
   2 — PRODUCT MATCH CARD
   Mobile: product image above (smaller) / copy below.
   Desktop: image left 36% / copy right 64%.
   ══════════════════════════════════════════════════════════════ */
.v8-product-card {
...
.v8-product-img-col {
  background: #F7F1E6;
  min-height: 150px; /* MOBILE: reduced from 180px — smaller image area */
  display: flex; align-items: center; justify-content: center; padding: 16px;
}
.v8-product-img {
  width: 100%;
  max-width: 110px; /* MOBILE: reduced from 140px */
...
/* ══════════════════════════════════════════════════════════════
   5 — COMPLIANCE NOTE (FDA disclaimer only — no unverified badges)
   ══════════════════════════════════════════════════════════════ */
.v8-compliance {
...
@media(min-width: 768px) {
  ...
  .v8-section-sm { padding: 48px 40px; } /* desktop: same as full section */
  .v8-h2         { font-size: 27px; }

  /* Hero — unchanged from v10 */
  ...
  /* Product card — desktop sizes restored */
...
<div class="vvp8">
  ...
  <!-- ════════════════════════════════════════════════════════════
       2 — PRODUCT MATCH CARD
       Sits directly under hero. Smaller image on mobile.
  ...
  <!-- ════════════════════════════════════════════════════════════
       4 — SIMPLE RITUAL (3 steps) + COMPLIANCE NOTE
       No unverified trust badges. FDA disclaimer only.
       ════════════════════════════════════════════════════════════ -->
  <div class="v8-section v8-bg-cream">
    <div class="v8-inner">
      ...
      <!-- Compliance note only — no unverified trust badges -->
...
<!-- ============================================================
     End VV Inner Bloom Result Page v13 Balanced Mobile Hero
     AUTO_PUBLISH=false | REQUIRE_HUMAN_APPROVAL=true
     ============================================================ -->
```
